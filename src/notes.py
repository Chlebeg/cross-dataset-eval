...
USED_FEATURES = ["frame.len", "radiotap.length", "radiotap.dbm_antsignal", "wlan.duration",
...
# csv_files_trn = glob.glob(os.path.join(AWID2_DIR + "AWID-CLS-R-Trn/"))
# csv_files_tst = glob.glob(os.path.join(AWID2_DIR + "AWID-CLS-R-Tst/"))

# csv_files_trn = os.listdir(AWID2_DIR + "AWID-CLS-FR-Trn/preproc")
# csv_files_tst = os.listdir(AWID2_DIR + "AWID-CLS-FR-Tst/preproc")

# trn_list = [pd.read_csv(os.path.join(AWID2_DIR + "AWID-CLS-FR-Trn/preproc", file), header=0) for file in csv_files_trn]
# tst_list = [pd.read_csv(os.path.join(AWID2_DIR + "AWID-CLS-FR-Tst/preproc", file), header=0) for file in csv_files_tst]

# awid2_trn = pd.concat(trn_list, ignore_index=True)
# awid2_tst = pd.concat(tst_list, ignore_index=True)

# awid2_trn.fillna(0, inplace=True)
# awid2_tst.fillna(0, inplace=True)
...
def preprocessAwid2(dataset):
    ...
    # output_file_path = os.path.join("..", TRN_FOLDER+"-pre", "1")
    # dataset.to_csv(output_file_path, index=False)
    # return dataset

awid2_trn = pd.read_csv(TRN_FOLDER+"/1")
awid2_trn.columns = awid2_columns

preprocessAwid2(awid2_trn)

df = pd.read_csv(TRN_FOLDER+"-pre/1")
# for x in df.columns:
#     print(df[x].dtype)
#     print(f"-----{x}-----\n{df[x].value_counts()}")
#     print(f"\n")
...
# print(awid2_trn.columns)
# print(awid2_tst.columns)

# trn_X = awid2_trn[awid2_trn.columns.remove("class")]
# trn_Y = awid2_trn["class"]
# tst_X = awid2_tst[awid2_tst.columns.remove("class")]
# tst_Y = awid2_tst["class"]
